...
template <class Driver> class LTC2874 {
public:
  LTC2874(Driver &driver) : driver_(driver) {}

  uint8_t read(Register reg) {
    uint8_t command = to_underlying(Command::READ) | (to_underlying(reg) << 1);
    driver_.spi_cs_low();
    driver_.spi_transmit(command);
    uint8_t res = driver_.spi_transmit(0);
    driver_.spi_cs_high();
    return res;
  }
  ...
  void write(Register reg, uint8_t data, bool update = false) {
    ...
    driver_.spi_cs_low();
    driver_.spi_transmit(command);
    driver_.spi_transmit(data);
    driver_.spi_cs_high();
  }

  void update() {
    driver_.spi_cs_low();
    driver_.spi_transmit(to_underlying(Command::UPDATE));
    driver_.spi_transmit(0x0);
    driver_.spi_cs_high();
  }

  void reset() {
    driver_.spi_cs_low();
    driver_.spi_transmit(to_underlying(Command::RESET));
    driver_.spi_transmit(0x0);
    driver_.spi_cs_high();
  }

  // Bitwise
  template <class Bit> void write_bit_raise(Register reg, Bit bit, bool update = false) {
    uint8_t data = read(reg);
    data |= to_underlying(bit);
    write(reg, data, update);
  }

  template <class Bit> void write_bit_lower(Register reg, Bit bit, bool update = false) {
    uint8_t data = read(reg);
    data &= ~to_underlying(bit);
    write(reg, data, update);
  }
